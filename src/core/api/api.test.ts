...
const getUsersUrl = `${baseURL}/search/users?q=test+type:org&page=1&per_page=10`;
const getUserDetailsUrl = `${baseURL}/orgs/test`;
...
describe('GithubApi class performs correct requests', () => {
  ...
  beforeAll(() => {
    ...
    mock.onGet(getUsersUrl).reply(200, mockOrgsResponse, {
    ...
    mock.onGet(getUserDetailsUrl).reply(200, mockDetailsResponse);
  });
  ...
  it('successfully /GET users from GitHub API', async () => {
    const data = await githubApi.getUsers({ query: 'test' });
    expect(data).toMatchObject(mockOrgsResponse);
  });

  it('successfully /GET user details from GitHub API', async () => {
    const data = await githubApi.getUser('test');
